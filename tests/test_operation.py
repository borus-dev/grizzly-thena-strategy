...
def test_operation_half_withdraw(
    chain, token, vault, strategy, gov,user, amount, RELATIVE_APPROX, thenaReward, thenaReward_whale
):
    # Deposit to the vault
    user_balance_before = token.balanceOf(user)
    token.approve(vault.address, amount, {"from": user})
...
def test_claim_rewards(
    ...
    assert rewardsBeforeTend == rewardsAfterTend # Tend here only deposits the Lp into masterChef
    # assert rewardsAfterTend < 1e18
...
def test_sweep(gov, vault, strategy, token, user, userWithDAI, amount, dai):
    ...
    assert dai.balanceOf(gov) == transferAmount + before_balance

def test_autocompounding(chain, vault, strategy, token,gov, amount, user, RELATIVE_APPROX,  thenaReward, thenaReward_whale):
    # Deposit to the vault
    user_balance_before = token.balanceOf(user)
    token.approve(vault.address, amount, {"from": user})
    vault.deposit(amount, {"from": user})
    ...
    for x in range(7):
        time = 86400 * 7 # 1 day of running the strategy
        util.airdrop_rewards(strategy, thenaReward, thenaReward_whale)
        chain.sleep(3600)
        chain.mine(1)
        ...
        assert stratPrevTendAssets == currentStratAssets # Tend only deposits LpTokens in masterChef
    ...
    assert pytest.approx(userProfit, rel=10e-3) == userExpectedProfit

def test_multiple_harvests(chain, gov,vault, strategy, token, amount, user, RELATIVE_APPROX, thenaReward, thenaReward_whale):
    # Deposit to the vault
    user_balance_before = token.balanceOf(user)
    token.approve(vault.address, amount, {"from": user})
    vault.deposit(amount, {"from": user})
...
def test_correct_APR(chain,gov, vault, strategy, token, amount, user,  thenaReward, thenaReward_whale):
    # Deposit to the vault
    token.approve(vault.address, amount, {"from": user})
    vault.deposit(amount, {"from": user})
    assert token.balanceOf(vault.address) == amount
...
def test_correct_APR_1_day(chain,gov, vault, strategy, token, amount, user, thenaReward, thenaReward_whale):
    # Deposit to the vault
    user_balance_before = token.balanceOf(user)
    token.approve(vault.address, amount, {"from": user})
    vault.deposit(amount, {"from": user})
